...
$rcv = json_decode($_SESSION['rcvjson']);
for($i=0;$i<intval($rcv->cnt);$i++){
  if($i>0){
    $tmp = 'female' . $i;
    $rcvfemales[] = array($tmp,$rcv->$tmp);
  }else{
    $rcvfemales[] = array('female',$rcv->female);
  }
}
...
// //radiobuttonの受信
if(isset($rcv->marriged)&&!empty($rcv->marriged)) {
  $marriged = json_encode($rcv->marriged);
}

// //checkboxの受信
if (isset($rcv->bijin) && is_array($rcv->bijin)) {
  $features= json_encode($rcv->bijin);
}

if (isset($rcv->bijintext) && !empty($rcv->bijintext)) {
  $bijintext= json_encode($rcv->bijintext);
}
?>
<style>
  .wrap {
                    ...
                    <div class="female-group">
                        <select id="female" name="female" style="width:200px;height: 30px;font-size:20px;margin-bottom:30px;">
                            <?php foreach($results as $data): ?>
                                <option id="<?php echo $data->getFemaleNumber(); ?>" value="<?php echo $data->getFemaleNumber(); ?>"><?php echo $data->getFemaleName() ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
